	...
	if len(tickerResp) > 0 {
		for _, ticker := range tickerResp {
			...
			*/
			...
			megaUpdateArray = append(megaUpdateArray, "('"+ticker.Symbol+"','"+strTime+"','"+ticker.QuoteVolume+"','1')")
		}
	} else {
		fmt.Println("Ошибка: нет данных в тикере")
	}
...
func updateTickers(megaUpdateString string) {
	...
	query2 := "REPLACE INTO `mexcs`(`symbol`,`updated_at`,`quote_volume`,`active`) VALUES" + megaUpdateString //	Запрос создадим
	//fmt.Println(query2)
